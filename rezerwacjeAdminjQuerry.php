...
<script>
    function check () {
        var car = document.getElementById("car");
        var selectedCar = car.options[car.selectedIndex];
        var id = selectedCar.value;
        var dataod = $("#dataod").val();
        var datado = $("#datado").val();
        $("#clear").html("");
        $("#clear1").html("");
        if(dataod > datado){
            $("#clear1").html("Data rozpoczęcia nie może być później niż data zakończenia");
        }  
        else{
        $.ajax({
            url:"ajax.php",    //the page containing php script
            type: "post",    //request type,
            data: {"idcar": id, "dataod": dataod, "datado": datado },
                ...
                <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 text-light">
                    ...
                    <div class="col-md-5 mx-auto text-center">
                        ...
                        <form method="post" action="">
                            ...
                            <div class="form-floating m-3 text-dark">
                                <input type="date" class="form-control" id="datado" name="datado" onchange="check()" required pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" min="<?php echo date('Y-m-d'); ?>">
